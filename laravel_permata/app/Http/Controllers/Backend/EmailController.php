...
class EmailController extends Controller
{
    ...
    {   
        $user       = Anggota::whereNotNull("email")->whereNotNull("tanggal_lahir")->whereRaw("MONTH(tanggal_lahir) = ".date('m'))->whereRaw("DAY(tanggal_lahir) = ".date('d'))->where("email","harisaginting@gmail.com")->get()->toArray();
        foreach ($user as $key => $value) {
             $model     = new Anggota();
             $data      = $model->get_anggota($value["email"]);
             $dataArr   = json_decode(json_encode($data),true); 
